...
// FIX: `marked.use({ highlight: ... })` and `langPrefix` are incorrect for newer versions of marked. 
// Replaced with a custom renderer for highlight.js integration as per marked's documentation.
const renderer = new marked.Renderer();
// FIX: Corrected renderer.code signature. Newer versions of `marked` pass a single token object
// to renderer methods instead of multiple arguments.
renderer.code = ({ text, lang }: { text: string; lang?: string }) => {
  // The `escaped` parameter is noted but unused, as highlight.js handles its own escaping.
        ...
        <div className={bubbleClasses}>
          {message.isLoading ? (
            <div className="flex items-center space-x-1.5">
              {/* FIX: Corrected typo in class names from w-1.S to w-1.5 */}
